...
//    //                {                   
//    //                    $arResult["ResolutionComissionFiles"][$i]["errorCode"]=iconv('UTF-8','windows-1251',$objFileItem->errorCode);
//    //                    $arResult["ResolutionComissionFiles"][$i]["linkID"]=iconv('UTF-8','windows-1251',$objFileItem->linkID);
//    //                    $arResult["ResolutionComissionFiles"][$i]["fileSize"]=iconv('UTF-8','windows-1251',$objFileItem->fileSize);
//    //                    $arResult["ResolutionComissionFiles"][$i]["fileFormat"]=iconv('UTF-8','windows-1251',$objFileItem->fileFormat);
//    //                    $arResult["ResolutionComissionFiles"][$i]["loadLink"]=iconv('UTF-8','windows-1251',$objFileItem->loadLink);   //ссылка на файл сейчас не используется
//    //                    $i++;
//    //                }
//    //            }
...
//    //            {
//    //                //ИЛИ ОБЪЕКТ
//    //                $arResult["ResolutionComissionFiles"][$i]["errorCode"]=iconv('UTF-8','windows-1251',$arSoapResponse["return"]->errorCode);
//    //                $arResult["ResolutionComissionFiles"][$i]["linkID"]=iconv('UTF-8','windows-1251',$arSoapResponse["return"]->linkID);
//    //                $arResult["ResolutionComissionFiles"][$i]["fileSize"]=iconv('UTF-8','windows-1251',$arSoapResponse["return"]->fileSize);
//    //                $arResult["ResolutionComissionFiles"][$i]["fileFormat"]=iconv('UTF-8','windows-1251',$arSoapResponse["return"]->fileFormat);
//    //                $arResult["ResolutionComissionFiles"][$i]["loadLink"]=iconv('UTF-8','windows-1251',$arSoapResponse["return"]->loadLink); //ссылка на файл сейчас не используется
//    //            }
//    //            else
//    //            {
...
//    //            if(!empty($arResult["ResolutionComissionFiles"]) && $arResult["ResolutionComissionFiles"][0]["errorCode"]==0)
//    //            {
//    //                $sLocalFileName = $arResult["ResolutionComissionFiles"][0]["linkID"].".".$arResult["ResolutionComissionFiles"][0]["fileFormat"];
//    //                $sServerFileName = $arResult["ResolutionComissionFiles"][0]["linkID"]; //на сервере файлы хранятся без расширения
//    //                $sLocalDirName =  "ResolutionComissionFiles"; //путь к директории пробуем менять
...
//     -------------------------------------------------------*/  
global $USER;
if ( CSite::InGroup( array(1,5) )){//проверка на принадлежность к группе
...
//да
{
    //проверяем пользователя по нашей базе
   // if(  
      //  ($_REQUEST["login"]=="info@alerio.ru" && $_REQUEST["password"]=="ipu5Mixa") ||                   /*Реководство Алерио*/
...
else
{
    ...
    $bErrorNotExists = true;        //флаг отсутствия ошибок
    $sWSDLPath="http://77.234.204.251:18180/Service4SaitRazreshenieNaStroitelstvo.asmx?WSDL";           //путь к WSDL Службы
    $sWebServiceName="PackResolutionCommissioning";                                                           //имя web-сервиса
    $sParamName="filter";                                                                               //наименование параметра soap-запроса
    $arSoapResponse = array();     //массив ранения результата ответа
    ...
    if(!empty($_REQUEST["id"])) 
    {
        ...
        //НЕТ
        {
            ...
            if(!empty($arResult["ResolutionComissionFiles"]) && $arResult["ResolutionComissionFiles"][0]["errorCode"]==0)
            {
                 ...
                 *  --Конец--16.07.2014 10:56
                 -------------------------------------------------------*/  
                //header("Location: http://expertiza/reestry/$arParams[BLOCK_URL]");
                die(); 
            }
        }
