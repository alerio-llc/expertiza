$(function() {
	...
	$('#ReestrZakluchenieExpFilterForm').click(function() {
		//удаляем предыдущие ошибки при клике на любой элемент формы
		$("#ReestrZakluchenieExpFilterForm p.error").remove();
		$("#ReestrZakluchenieExpFilterForm label.error").removeClass("error");
		$("#ReestrZakluchenieExpFilterForm input.error").removeClass("error");
		
	});
	
	
	
	$('#ReestrZakluchenieExpFilterForm').submit(function() {
		...
		{
			$("#ReestrZakluchenieExpFilterForm").prepend('<p class="error" >Необходимо задать хотя бы один фильтр!</p>');
			$("#ReestrZakluchenieExpFilterForm input[type='text']").addClass("error");
			//удаляем количество найденных в прошлый раз документов
			$("#CountResult").remove();
			//удаляем таблицу, если она ранее была выведена
			$("#ReestrHeader").remove();	//шапка таблицы
