...
$(function() 
{
	...
	// TODO: подумать, нужно ли писать предупреждение об отключекнном JavaScript, а пока закомментируем 
...
//	$("#JavaScriptIsPresent").css("display", "block");
		...
		if(sObjectName!="" || sObjectAddress!="" || sObjectBuilder!="")
		{
			...
			{	
				...
				window.win.postMessage(
						sRequest,						
						//"http://testmap.alerio.ru" 	
						//"http://toris.iac.spb.ru" 			
						"http://gis.iac.spb.ru/stroinadzor"			
					);
	...
	function listener(event){
		...
		if ( event.origin !== "http://gs.iac.spb.ru/stroinadzor" ) //проверка домена с которого пришло сообщение
			return;
